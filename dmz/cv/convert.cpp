...
DMZ_INTERNAL void llcv_lineardown2_1d_u8_c(IplImage *src, IplImage *dst) {
  // lgq 函数cvResize 重新调整图像src（或它的ROI），使它精确匹配目标dst（或其ROI）。
  // 这里需要说明的是，cvResize可以用来调整3通道图像（如RGB图像）和单通道图像的大小。
...
DMZ_INTERNAL void llcv_norm_convert_1d_u8_to_f32_c(IplImage *src, IplImage *dst) {
  cvConvertScale(src, dst, 1.0f / 255.0f, 0);//lgq 对每个源数组元素执行线性变换：
  cvNormalize(dst, dst, 0.0f, 1.0f, CV_MINMAX, NULL);//找到数组（或两个数组）的范数，差异范数或相对差异范数
}
