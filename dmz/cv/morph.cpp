...
static inline void vec_morph3_1d_u8_q(const uint8_t *src, uint8_t *dst) {
  asm volatile
  (
   // load src pixel vectors (src - 1, src, src + 1) lgq 载入src_data 像素向量
   "mov r0, %[src]" "\n\t"    //r0 = src
   "sub r0, r0, #1" "\n\t"    //r0 = r0 - 1
   "vld1.8 {q0}, [r0]" "\n\t" //从r0加载8个元素，到寄存器q0
   "add r0, r0, #1" "\n\t"    // r0 = r0 -1
...
//lgq 计算兴趣区域的大小
  ...
  bool done = false;
  while(!done) {
    // calc using q registers 使用q寄存器计算
    ...
    if(dst_index + 1 == dst_size.width) {
      ...
      // backtrack to handle leftovers 回溯处理剩余的
...
DMZ_INTERNAL void llcv_morph_grad3_2d_cross_u8(IplImage *src, IplImage *dst) {
  ...
  for(int iter = 0; iter < TIME_MORPH2D_TIMING_ITERATIONS; iter++) {
    ...
    llcv_morph_grad3_2d_cross_u8_c_neon(src, dst);//lgq 做了什么？
