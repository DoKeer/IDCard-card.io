...
static float number_grad_sum_pattern[19] = {//lgq 数量总和模型
...
//          std::cerr << ">>>>lgq center_of_number + 19 ==  " << center_of_number + 19 << "\n";
...
//      std::cerr << ">>>>lgq in_bounds ==  " << in_bounds << "\n";
      ...
      if(in_bounds) {
        ...
        if(score < best.score) {
          ...
          std::cerr << ">>>>lgq best.score ==  " << score << "\n";
          std::cerr << ">>>>lgq number_width ==  " << width << "\n";
          std::cerr << ">>>>lgq pattern_offset ==  " << offset << "\n";
...
DMZ_INTERNAL NHorizontalSegmentation best_n_hseg(IplImage *y_strip, NVerticalSegmentation vseg) {
  ...
  llcv_morph_grad3_2d_cross_u8(y_strip, grad);//lgq 做了什么？
  ...
  cvNormalize(grad_sum, grad_sum, 0.0f, 1.0f, CV_MINMAX, NULL);//lgq 归一化 0 - 1
  ...
  std::cerr << ">>>>lgq grad_sum_data +++  ==  " << grad_sum_data << "\n";
