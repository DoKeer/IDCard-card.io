...
DMZ_INTERNAL void scan_card_image(IplImage *y, bool collect_card_number, bool scan_expiry, FrameScanResult *result) {
  ...
  result->usable = false;
  //lgq 获取图像卡号区域的y坐标，卡号长度，score,类型 visalike AmericanExpresslike
  ...
  if (result->vseg.y_offset < kFlipVSegYOffsetCutoff) { //要是卡号靠近卡片上部，标识出卡片可能拿倒了
  ...
  if (collect_card_number) {
    ...
    result->hseg = best_n_hseg(y, result->vseg); //x水平方向获取卡号区域，
    //lgq hseg score质量上不够可靠，这是图形管道最难的部分，正在寻求其他方法
    ...
    std::cerr << ">>>>lgq number_score  ==  " <<number_score << "\n";

    result->usable = number_score < kMaxNumberScoreDelta;//lgq number_score < 3

    std::cerr << ">>>>lgq result->usable  ==  " <<result->usable << "\n";
