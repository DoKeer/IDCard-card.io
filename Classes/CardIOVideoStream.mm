...
- (BOOL)setTorchModeOnWithLevel:(float)torchLevel {
  ...
  BOOL success = [self changeCameraConfiguration:^{//lgq 闪光灯手电筒
    NSError *error;
  ...
  @autoreleasepool {
    ...
    if (self.running) {
      ...
      if (!self.currentlyAdjustingFocus) {
        ...
        if (exifDict != nil) {
          frame.isoSpeed = [exifDict[@"ISOSpeedRatings"][0] integerValue]; //感光度
          frame.shutterSpeed = [exifDict[@"ShutterSpeedValue"] floatValue];//lgq 快门速度
        }
        else {
...
//      [[NSString stringWithFormat:@"%0.2f", fps] drawInRect:CGRectIntegral(rect)
