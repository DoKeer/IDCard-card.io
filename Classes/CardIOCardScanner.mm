...
- (void)addFrame:(CardIOIplImage *)y
 ...
 brightnessScore:(float)brightnessScore
  ...
  //lgq 把_scannerState和result指针传入识别方法，开始识别图像边界和卡号，保存识别状态scannerState。
...
- (CardIOReadCardInfo *)cardInfo {//return nil 继续识别
  if (self.scanIsComplete) {
    return self.cardInfoCache;//lgq 识别过了，直接返回
  }
  ...
  if(self.cardInfoCacheDirty) {
    ...
    scanner_result(&_scannerState, &result);//lgq获取卡号识别结果
    if(result.complete) {
